...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (cropDataForm) {
    ...
    cropDataForm.addEventListener('submit', (e) => {
      ...
      if (missingFields) {
        window.showToast('error', 'Please fill in all required fields', 'The fields marked with * are required.');
        return;
      }
      ...
      setTimeout(() => {
        ...
        window.showToast('success', 'Data processed successfully', 'Redirecting to results page...');
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
      ...
      setTimeout(() => {
        ...
        window.showToast('success', 'Message Sent', 'Thank you for your message. We will get back to you soon!');
...
function generatePredictionResults(data) {
  ...
  const factors = cropFactors[cropType] || cropFactors.rice;
  ...
  predictedYield += parseFloat(data.nitrogen || 0) * factors.nFactor;
  predictedYield += parseFloat(data.phosphorus || 0) * factors.pFactor;
  predictedYield += parseFloat(data.potassium || 0) * factors.kFactor;
  ...
  const yieldRangeElement = document.getElementById('yieldRange');
  ...
  if (yieldValueElement) {
    yieldValueElement.textContent = predictedYield.toLocaleString();
  }
  
  if (yieldRangeElement) {
    yieldRangeElement.textContent = `${lowRange.toLocaleString()} - ${highRange.toLocaleString()} kg/ha`;
  }
  
  // Animate confidence bar
  setTimeout(() => {
    if (confidenceFillElement) {
      confidenceFillElement.style.width = `${confidenceScore}%`;
    }
    
    if (confidenceValueElement) {
      confidenceValueElement.textContent = `${confidenceScore}%`;
    }
  }, 300);
...
function generateImpactFactors(data, predictedYield) {
  ...
  // Soil Health factor
  let nutrientLevel = parseInt(data.nitrogen || 0) + parseInt(data.phosphorus || 0) + parseInt(data.potassium || 0);
  const soilFactor = {
...
function generateRecommendations(data, predictedYield) {
  ...
  if (!recommendationsList || !data) return;
  ...
  const nitrogen = parseInt(data.nitrogen || 0);
  const phosphorus = parseInt(data.phosphorus || 0);
...
function generateInsights(data) {
  ...
  const weatherInsightEl = document.getElementById('weatherInsight');
  const soilInsightEl = document.getElementById('soilInsight');
  const growthInsightEl = document.getElementById('growthInsight');
  const yieldInsightEl = document.getElementById('yieldInsight');
  
  if (weatherInsightEl) weatherInsightEl.textContent = generateWeatherInsight(data);
  if (soilInsightEl) soilInsightEl.textContent = generateSoilInsight(data);
  if (growthInsightEl) growthInsightEl.textContent = generateGrowthInsight(data);
  if (yieldInsightEl) yieldInsightEl.textContent = generateYieldInsight(data);
...
function generateSoilInsight(data) {
  ...
  const ph = parseFloat(data.soilPh || 0);
...
function generateYieldInsight(data) {
  ...
  const tempFactor = 1 + (parseFloat(data.temperature || 25) - 25) * 0.01;
  // Rainfall adjustment
  const rainFactor = 1 + (parseFloat(data.rainfall || 150) - 150) * 0.001;
  // Nutrient adjustment
  const nutrientSum = parseInt(data.nitrogen || 0) + parseInt(data.phosphorus || 0) + parseInt(data.potassium || 0);
...
function generateTimelineRecommendations(data) {
  ...
  if (parseInt(data.nitrogen || 0) < 60) {
    recommendations.push({
  ...
  if (parseInt(data.phosphorus || 0) < 30) {
    recommendations.push({
  ...
  if (parseFloat(data.soilPh || 0) < 5.5) {
    recommendations.push({
  ...
  if (parseFloat(data.rainfall || 0) < 80) {
    recommendations.push({
  ...
  } else if (parseFloat(data.rainfall || 0) > 200) {
    recommendations.push({
    ...
    datasets: [
      {
        ...
        data: [parseFloat(data.temperature || 0), parseFloat(data.rainfall || 0) / 10, parseFloat(data.humidity || 0)],
    ...
    datasets: [
      {
        ...
        data: [
          parseInt(data.nitrogen || 0),
          parseInt(data.phosphorus || 0),
          parseInt(data.potassium || 0)
        ],
        backgroundColor: [
          'rgba(124, 179, 66, 0.7)',
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (floatElements.length > 0) {
    floatElements.forEach(element => {
      // Create random animation details for variation
      const duration = 3 + Math.random() * 2; // Between 3-5s
      const distance = 5 + Math.random() * 10; // Between 5-15px
      
      element.style.animation = `float ${duration}s ease-in-out infinite`;
      
      // Add keyframes for floating animation if not already in stylesheet
  ...
  // Add pulse animation to elements with 'pulse' class
  const pulseElements = document.querySelectorAll('.pulse');
  if (pulseElements.length > 0) {
    pulseElements.forEach(element => {
      element.style.animation = 'pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite';
      
      // Add keyframes for pulse animation if not already in stylesheet
